...
class Mittens(MittensBase):
    ...
    def _fit(self, coincidence, weights, log_coincidence,
        ...
        train_iter = tqdm(range(self.max_iter))
        for iteration in train_iter:
            pred = self._make_prediction()
            gradients, error = self._get_gradients_and_error(
                pred, log_coincidence, weights)
            if error < self.tol:
                train_iter.set_description("Iteration {}: stopping with loss < self.tol".format(iteration))
                # self._progressbar("stopping with loss < self.tol", iteration)
                break
            train_iter.set_description("Iteration {}: error {:10.6f}".format(iteration, error))
            # self._progressbar("error {:10.6f}".format(error), iteration)
    ...
    def _get_gradients_and_error(self,
        ...
        # fix
        # sz = len(log_coincidence.data)
        # row = log_coincidence.row
        # col = log_coincidence.col
        # diffs = predictions.copy()
        # for i in range(sz):
        #     diffs[row[i]][col[i]] = predictions[row[i]][col[i]] - log_coincidence.data[i]
        #     diffs[col[i]][row[i]] = predictions[col[i]][row[i]] - log_coincidence.data[i]
        ...
        if self.mittens > 0:
            ...
            error += self.mittens * np.square(distance).sum()
