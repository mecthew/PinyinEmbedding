...
from doc import BASE_DOC, MITTENS_PARAM_DESCRIPTION
...
class MittensBase(object):
    ...
    def _initialize(self, coincidence):
        ...
        # fix
        # weights = np.zeros(coincidence.shape, dtype=np.float32)
        # sz = len(coincidence.data)
        # for i in range(sz):
        #     if coincidence.data[i] >= self.xmax:
        #         weights[coincidence.row[i]][coincidence.col[i]] = 1.0
        #     else:
        #         weights[coincidence.row[i]][coincidence.col[i]] = (coincidence.data[i] / self.xmax) ** self.alpha
    ...
    def _progressbar(self, msg, iter_num):
        ...
        if self.display_progress and \
            ...
            print("Iteration {}: {}".format(iter_num + 1, msg))
